...
class CourseRepository {
  ...
  async read(id: number) {
    ...
    const [rows] = await databaseClient.query<Rows>(
      "SELECT ca.name as `type`, c.title, c.corpus, c.is_active, t.name as `topic`, t.image as `image`, c.reward FROM course c JOIN category ca ON c.id_category = ca.id JOIN topic t ON t.id = c.topic_id WHERE c.id = ?",
      [id],
    );
  ...
  async readAll() {
    ...
    const [rows] = await databaseClient.query<Rows>(
      "SELECT c.id as `id`, ca.name as `type`, c.title, c.corpus, c.is_active, t.name as `topic`, t.image as `image`, c.reward FROM course c JOIN category ca ON c.id_category = ca.id JOIN topic t ON t.id = c.topic_id",
    );
